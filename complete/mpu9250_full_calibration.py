...
def gyro_cal():
    ...
    [readACCGYR() for ii in range(0, cal_size)]
    mpu_array = []  # imu array for gyro vals
    gyro_offsets = [0.0, 0.0, 0.0]  # gyro offset vector
    while True:
        try:
            _, _, _, wx, wy, wz = readACCGYR()  # read and convert mpu6050 data
        except:
            continue
        ...
        if np.shape(mpu_array)[0] == cal_size:
            for qq in range(0, 3):
                gyro_offsets[qq] = np.mean(
                    np.array(mpu_array)[:, qq])  # calc gyro offsets
            break
...
def get_accel():
    ax, ay, az, _, _, _ = readACCGYR()  # read and convert accel data
    return ax, ay, az
...
def accel_cal():
    ...
    cal_directions = ["upward", "downward",
                      "perpendicular to gravity"]  # direction for IMU cal
        ...
        for direc_ii, direc in enumerate(cal_directions):
            ...
            while len(mpu_array) < cal_size:
                try:
                    ...
                    continue
            ax_offsets[direc_ii] = np.array(
                mpu_array)[:, cal_indices[qq]]  # offsets for direction
...
def mag_cal():
    ...
    for qq, ax_qq in enumerate(mag_cal_axes):
        try:
            input(
                "-"*8+" Press Enter and Start Rotating the IMU Around the "+ax_qq+"-axis")
        ...
        while True:
            try:
                mx, my, mz = readMAG()  # read and convert AK8963 magnetometer data
            except KeyboardInterrupt:
                break
            except:
                continue
            mag_array.append([mx, my, mz])  # mag array
        # throw away first few points (buffer clearing)
        mag_array = mag_array[20:]
        mag_cal_rotation_vec.append(mag_array)  # calibration array
        print("Sample Rate: {0:2.0f} Hz".format(
            len(mag_array)/(time.time()-t0)))
    ...
    for mag_ii, mags in enumerate(mag_cal_rotation_vec):
        ...
        # append to offset
        ak_fit_coeffs.append([x_0, y_0][indices_to_save[mag_ii]])
...
def mpu_plot_test():
    ...
    y_labels = ['Acceleration [g]',
                'Angular Velocity [$^\circ/s$]', 'Magnetic Field [uT]']
    ...
    # imu time series title
    axs[0].set_title('Calibrated MPU9250 Time Series Plot')
    ...
    ax_legs = [axs[tt].legend()
               for tt in range(0, len(axs))]  # legends for axes
    ax_bgnds = [fig.canvas.copy_from_bbox(
        axs[tt].bbox) for tt in range(0, len(axs))]  # axis backgrounds
    ...
    while True:
        try:
            ax, ay, az, wx, wy, wz = readACCGYR()  # read and convert mpu6050 data
            mx, my, mz = readMAG()  # read and convert AK8963 magnetometer data
        except:
            continue

        mpu_array[0:-1] = mpu_array[1:]  # get rid of last point
        mpu_array[-1] = [ax, ay, az, wx, wy, wz, mx,
                         my, mz]  # update last point w/new data

        if ii_iter == 100:
            [fig.canvas.restore_region(ax_bgnds[tt]) for tt in range(
                0, len(ax_bgnds))]  # restore backgrounds
            for ii in range(0, 9):
                if ii in range(0, 3):
                ...
                if ii in range(3, 6):
                    # update gyro data
                    lines[ii].set_ydata(
                        np.array(mpu_array[:, ii])-cal_offsets[ii])
                    axs[1].draw_artist(lines[ii])  # update gyro plot
                if ii in range(6, 9):
                    jj = ii-6  # index offsetted to 0-2
                    # raw x-variable
                    x = np.array(mpu_array[:, cal_rot_indicies[jj][0]])
                    # raw y-variable
                    y = np.array(mpu_array[:, cal_rot_indicies[jj][1]])
                    # x-var for heading
                    x_prime = x - cal_offsets[cal_rot_indicies[jj][0]]
                    # y-var for heading
                    y_prime = y - cal_offsets[cal_rot_indicies[jj][1]]
                    x_prime[np.isnan(x)] = np.nan
                    y_prime[np.isnan(y)] = np.nan
                    r_var = np.sqrt(np.power(x_prime, 2.0) +
                                    np.power(y_prime, 2.0))  # radius vector
                    # angle vector for heading
...
# lines[int(ii+jj+1)].set_data(x_prime,y_prime) # update heading data
                    ...
                    # update heading plot
                    axs[3].draw_artist(lines[int(ii+jj+1)])
            [axs[tt].draw_artist(ax_legs[tt])
             for tt in range(0, len(ax_legs))]  # update legends
            [fig.canvas.blit(axs[tt].bbox)
             for tt in range(0, len(ax_legs))]  # update axes
...
if __name__ == '__main__':
    ...
    if not start_bool:
        ...
        if str(re_cal_bool) == "1":
            ...
            with io.open(cal_filename, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile, delimiter=',')
                for param_ii in range(0, len(cal_offsets)):
                    if param_ii > 2:
                        writer.writerow(
                            [cal_labels[param_ii], cal_offsets[param_ii]])
                    else:
                        writer.writerow(cal_labels[param_ii] +
                                        [ii for ii in cal_offsets[param_ii]])
        else:
            # read calibration coefficients from file
            with io.open(cal_filename, 'r', newline='') as csvfile:
                reader = csv.reader(csvfile, delimiter=',')
                iter_ii = 0
                for row in reader:
                    if len(row) > 2:
                        row_vals = [float(ii)
                                    for ii in row[int((len(row)/2)+1):]]
